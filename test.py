...
from models.unitt_stereo import UniTTStereo
from models.refinenet_based_head import PixelwiseTaskWithDPT

from stereo.criterion import *
from stereo.datasets_stereo import get_test_datasets_stereo
from stereo.engine import tiled_pred

from stereo.datasets_stereo import vis_disparity


def get_args_parser():
    parser = argparse.ArgumentParser('Test models on stereo', add_help=False)
    # important argument 
    ...
    # tiling
    parser.add_argument('--tile_conf_mode', type=str, default='',
...
def _load_model_and_criterion(model_path, do_load_metrics, device):
    ...
    print('model_args:', ckpt_args.model_args)
    model = UniTTStereo(head, **ckpt_args.model_args)
...
def _save_batch(pred, gt, pairnames, dataset, task, save, outdir, time, submission_dir=None):
    for i in range(len(pairnames)):
        ...
        if 'visu' in save:
            if task == 'stereo':
                ...
                if gt is not None:
                    ...
                    gt = gt.squeeze(1)
                    m = gt[mask].min().cpu().numpy()
                    M = gt[mask].max().cpu().numpy()
        ...
        if 'err10' in save:
            ...
            L1err = torch.sum( abs(gti-predi), dim=0)

            valid = torch.isfinite(gti[0,:,:])
            L1err[~valid] = 0.0
            L1err[L1err > 1.0] = 1.0
            L1err[L1err < 1.0] = 0.0
            red = (L1err*255.0).to(dtype=torch.uint8)[:,:,None]
            zer = torch.zeros_like(red)
            imgerr = torch.cat( (red,zer,zer), dim=2)
            imgerr = imgerr.cpu().numpy()
            ...
            zer = torch.zeros_like(red)
            imgerr = torch.cat( (red,zer,zer), dim=2)
            nonzero_r = (imgerr[:,:,0]>0)
            imgerr[:,:,0][~nonzero_r] = 255
            imgerr[:,:,1][~nonzero_r] = 255
            imgerr[:,:,2][~nonzero_r] = 255
            imgerr = imgerr.cpu().numpy()
            Image.fromarray(imgerr).save(fname + '_err10.png')
            """
        ...
        if 'submission' in save:
            ...
            predi_np = predi.permute(1, 2, 0).squeeze(
                2).cpu().numpy()  # transform into HxW for stereo
...
def main(args):
    ...
    model, metrics, cropsize, with_conf, task, tile_conf_mode = _load_model_and_criterion(args.model,
                                                                                          'metrics' in args.save,
                                                                                          device)
    if args.tile_conf_mode == '': args.tile_conf_mode = tile_conf_mode

    # load the datasets
    datasets = (get_test_datasets_stereo)(args.dataset)
    dataloaders = [
    ...
    for i, dataloader in enumerate(dataloaders):
        ...
        for data_iter_step, (image1, image2, gt, pairnames) in enumerate(tqdm(dataloader)):
            ...
            do_flip = (task == 'stereo' and dstr.startswith('Spring') and any("right" in p for p in
                                                                              pairnames))  # we flip the images and will flip the prediction after as we assume img1 is on the left
            ...
            with torch.inference_mode():
                pred, _, _, time, _, _, _, _ = tiled_pred(model, None, image1, image2, None if dataset.name == 'Spring' else gt,
                                              conf_mode=args.tile_conf_mode, overlap=args.tile_overlap, crop=cropsize,
                                              with_conf=with_conf, return_time=True)
                ...
                if any(k in args.save for k in ['pred', 'visu', 'err10', 'submission']):
                    _save_batch(pred, gt, pairnames, dataset, task, args.save, outdir, time,
