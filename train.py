...
from stereo.consistency_loss import ConsistencyLoss
from utils.misc import NativeScalerWithGradNormCount as NativeScaler
from models.unitt_stereo import UniTTStereo, croco_args_from_ckpt
from models.pos_embed import interpolate_pos_embed
from models.refinenet_based_head import PixelwiseTaskWithDPT

from stereo.datasets_stereo import get_train_dataset_stereo, get_test_datasets_stereo
from stereo.engine import train_one_epoch, validate_one_epoch
from stereo.criterion import *
...
def get_args_parser():
    # prepare subparsers
    parser = argparse.ArgumentParser('Finetuning models on stereo or flow', add_help=False)
    ...
    # model
    add_arg('--crop', type=int, nargs='+', default_stereo=[352, 704], default_flow=[320, 384],
    ...
    # training
    add_arg('--mask_ratio', default=[0.90], nargs='*', type=float)
    add_arg('--norm_pix_loss', default=1, choices=[0, 1],
    ...
    add_arg('--img_per_epoch', type=int, default=None,
    ...
    add_arg('--start_from', type=str, default=None,
    ...
    add_arg('--ffn_adapt', default=False, action='store_true', help='whether activate AdaptFormer')
    add_arg('--ffn_num', default=64, type=int, help='bottleneck middle dimension')
    ...
    add_arg('--fulltune', default=False, action='store_true', help='full finetune model')
...
def main(args):
    ...
    metrics = (StereoMetrics)().to(device)
    criterion = eval(args.criterion).to(device)
    print('Criterion: ', args.criterion)
    print('Loading criterion2: MaskedMSE(norm_pix_loss={:s})'.format(str(bool(args.norm_pix_loss))))
    criterion2 = MaskedMSE(norm_pix_loss=bool(args.norm_pix_loss))
    criterion3 = ConsistencyLoss()

    # Prepare model
    assert os.path.isfile(args.pretrained)
    ckpt = torch.load(args.pretrained, 'cpu')
    model_args = croco_args_from_ckpt(ckpt)
    model_args['img_size'] = (args.crop[0], args.crop[1])
    model_args['mask_ratio'] = args.mask_ratio
    print('Model args: ' + str(model_args))
    args.model_args = model_args  # saved for test time
    # prepare head
    ...
    model = UniTTStereo(head, **model_args)
    ...
    if args.distributed:
        ...
        print("distributed training >>> gpu id: ", end="")
        print(args.gpu)
    ...
    if not args.resume and args.start_from:
        ...
        best_so_far = misc.load_model(args=args, model_without_ddp=model_without_ddp, optimizer=optimizer,
    ...
    train_dataset = (get_train_dataset_stereo)(args.dataset,crop_size=args.crop)
    ...
    if args.val_dataset == '':
        ...
        val_datasets = (get_test_datasets_stereo)(args.val_dataset)
        ...
        data_loaders_val = [
            DataLoader(val_dataset, batch_size=1, shuffle=False, num_workers=args.num_workers, pin_memory=True,
                       drop_last=False) for val_dataset in val_datasets]
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(model, criterion, criterion2, criterion3, metrics, data_loader_train, optimizer,
        ...
        if data_loaders_val is not None and args.eval_every > 0 and (epoch + 1) % args.eval_every == 0:
            ...
            val_stats = validate_one_epoch(model, criterion, metrics, data_loaders_val, device, epoch,
            ...
            if val_best <= best_so_far:
                ...
                misc.save_model(args=args, model_without_ddp=model_without_ddp, optimizer=optimizer,
                                loss_scaler=loss_scaler, epoch=epoch, best_so_far=best_so_far, fname='best')
        ...
        if args.output_dir and ((epoch + 1) % args.save_every == 0 or epoch + 1 == args.epochs):
            misc.save_model(args=args, model_without_ddp=model_without_ddp, optimizer=optimizer,
                            loss_scaler=loss_scaler, epoch=epoch, best_so_far=best_so_far, fname='last')
