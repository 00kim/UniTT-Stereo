...
class CroCoNet(nn.Module):
    ...
    def __init__(self,
                 ...
                 mask_ratio=[0.0, 0.5, 0.25, 1.0],  # ratios of masked tokens
        ...
        if pos_embed == 'cosine':
            ...
            enc_pos_embed = get_2d_sincos_pos_embed(enc_embed_dim, int(self.patch_embed.num_patches ** .5),
                                                    n_cls_token=0)
    ...
    def _decoder(self, feat1, pos1, masks1, feat2, pos2, return_all_blocks=False):
        ...
        if return_all_blocks:
            _out, out = out, []
            num=self.enc_depth+1
            for blk in self.dec_blocks:
                _out, out2, attn_score, cross_attn_score = blk(_out, out2, pos1, pos2, num)
                num+=1
                attn_scores.append(attn_score)
                cross_attn_scores.append(cross_attn_score)
                out.append(_out)
            out[-1] = self.dec_norm(out[-1])
        else:
            num = self.enc_depth + 1
            for blk in self.dec_blocks:
                out, out2, attn_score, cross_attn_score  = blk(out, out2, pos1, pos2, num)
                num+=1
                attn_scores.append(attn_score)
                cross_attn_scores.append(cross_attn_score)
            out = self.dec_norm(out)
        return out, attn_scores, cross_attn_scores
