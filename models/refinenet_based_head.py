...
from .fusion_block import DPTOutputAdapter
    ...
    def setup(self, croconet):
        ...
        if self.hooks_idx is None:
            if hasattr(croconet, 'dec_blocks'):  # encoder + decoder
                hooks_idx = {12:[13, 16, 19, 23], 24: [25, 28, 31, 35]}[croconet.enc_depth]
            self.hooks_idx = hooks_idx
        ...
        dim_tokens = [croconet.enc_embed_dim if hook < croconet.enc_depth else croconet.dec_embed_dim for hook in
                      self.hooks_idx]
