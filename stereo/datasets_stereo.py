...
dataset_to_root = {
    ...
    'ETH3DLowRes': '../../datasets/eth3d_lowres/',
    'Booster': '../../datasets/booster_gt/',
    'Middlebury2021': '../../datasets/Middlebury/2021/data/',
    'Middlebury2014': '../../datasets/Middlebury/2014/',
    'Middlebury2006': '../../datasets/Middlebury/2006/',
    'Middlebury2005': '../../datasets/Middlebury/2005/train/',
    'MiddleburyEval3': '../../datasets/Middlebury/MiddEval3/',
    'Spring': '../../datasets/spring/',
    'Kitti15': '../..//datasets/kitti2015/',
    'Kitti12': '../../datasets/kitti2012/',
    'TartanAir': '../../datasets/TartanAir',
    'MPISintel': '../../datasets/MPI-Sintel/',
    'DrivingStereo': '../../datasets/DrivingStereo/',
}
cache_dir = "./data/stereo/datasets_stereo_cache/"
...
class DrivingStereoDataset(StereoDataset):
    def _prepare_data(self):
        self.name = "DrivingStereo"
        self._set_root()
        assert self.split in ['sunny', 'cloudy', 'foggy', 'rainy']
        self.pairname_to_Limgname = lambda pairname: osp.join(self.root, pairname)
        self.pairname_to_Rimgname = lambda pairname: osp.join(self.root, pairname).replace('left-image', 'right-image')
        self.pairname_to_Ldispname = lambda pairname: osp.join(self.root, pairname).replace('left-image',
                                                                                            'disparity-map').replace(
            'jpg', 'png')
        self.pairname_to_str = lambda pairname: pairname[:-4]
        self.load_disparity = _read_drivingstereo_disp

    def _build_cache(self):
        # sunny
        sunnypairs = sorted(glob(self.root + 'sunny/left-image-half-size/*.jpg'))
        sunnypairs = list(map(lambda x: x[len(self.root):], sunnypairs))
        # cloudy
        cloudypairs = sorted(glob(self.root + 'cloudy/left-image-half-size/*.jpg'))
        cloudypairs = list(map(lambda x: x[len(self.root):], cloudypairs))
        # foggy
        foggypairs = sorted(glob(self.root + 'foggy/left-image-half-size/*.jpg'))
        foggypairs = list(map(lambda x: x[len(self.root):], foggypairs))
        # rainy
        rainypairs = sorted(glob(self.root + 'rainy/left-image-half-size/*.jpg'))
        rainypairs = list(map(lambda x: x[len(self.root):], rainypairs))

        # all
        tosave = {'sunny': sunnypairs,
                  'foggy': foggypairs,
                  'cloudy': cloudypairs,
                  'rainy': rainypairs,
                  }
...
class MPISintelDataset(StereoDataset):

    def _prepare_data(self):
        self.name = "MPISintel"
        self._set_root()
        assert self.split in [s + '_' + p for s in ['train', 'subval', 'subtrain'] for p in
                              ['cleanpass', 'finalpass', 'allpass']]
        self.pairname_to_Limgname = lambda pairname: osp.join(self.root, pairname[0],
                                                              'frame_{:04d}.png'.format(pairname[1]))
        self.pairname_to_Rimgname = lambda pairname: osp.join(self.root, pairname[0].replace('left', 'right'),
                                                              'frame_{:04d}.png'.format(pairname[1]))
        self.pairname_to_Ldispname = lambda pairname: osp.join(self.root, pairname[0].replace('/clean_left/',
                                                                                              '/disparities/').replace(
            '/final_left/', '/disparities/'), 'frame_{:04d}.png'.format(pairname[1]))
        self.pairname_to_str = lambda pairname: osp.join(pairname[0], 'frame_{:04d}'.format(pairname[1]))
        self.load_disparity = _read_sintel_disp

    def _build_cache(self):
        trainseqs = sorted(os.listdir(self.root + 'training/clean_left'))
        trainpairs = [(osp.join('training/', s, p), i) for s in ['clean_left/', 'final_left/'] for p in trainseqs for i
                      in
                      range(1, len(os.listdir(self.root + 'training/' + s + p)))]
        subvalseqs = ['temple_2']
        ...
        # assert len(trainpairs) == 1064 and len(subvalpairs) == 100 and len(
        #   subtrainpairs) == 964, "incorrect parsing of pairs in MPI-Sintel"
        tosave = {}
        tosave['train_cleanpass'] = trainpairs
        tosave['subval_cleanpass'] = subvalpairs
        tosave['subtrain_cleanpass'] = subtrainpairs
        for t in ['train', 'subval', 'subtrain']:
...
class TartanAirDataset(StereoDataset):
    ...
    def _prepare_data(self):
        ...
        self.pairname_to_Limgname = lambda pairname: osp.join(self.root, pairname[0],
                                                              'image_left/{:06d}_left.png'.format(pairname[1]))
        self.pairname_to_Rimgname = lambda pairname: osp.join(self.root, pairname[0],
                                                              'image_right/{:06d}_right.png'.format(pairname[1]))
        self.pairname_to_Ldispname = lambda pairname: osp.join(self.root, pairname[0],
                                                               'depth_left/{:06d}_left_depth.npy'.format(pairname[1]))
        self.pairname_to_str = lambda pairname: os.path.join(pairname[0][pairname[0].find('/'):],
                                                             '{:06d}'.format(pairname[1], pairname[2]))
        self.load_disparity = _read_tartanair_disp

    def _build_cache(self):
        seqs = sorted(os.listdir(self.root))
        pairs = [(osp.join(s, s, s, difficulty, Pxxx), int(a[:6]), int(a[:6]) + 1) for s in seqs for difficulty in
                 ['Easy', 'Hard'] for Pxxx in sorted(os.listdir(osp.join(self.root, s, s, s, difficulty))) for a in
                 sorted(os.listdir(osp.join(self.root, s, s, s, difficulty, Pxxx, 'image_left/')))[:-1]]
...
class CREStereoDataset(StereoDataset):
    ...
    def _build_cache(self):
        allpairs = [s + '/' + f[:-len('_left.jpg')] for s in sorted(os.listdir(self.root)) for f in
                    sorted(os.listdir(self.root + '/' + s)) if f.endswith('_left.jpg')]
...
class SceneFlowDataset(StereoDataset):
    ...
    def _prepare_data(self):
        self.name = "SceneFlow"
        self._set_root()
        assert self.split in ['train_finalpass', 'train_cleanpass', 'train_allpass', 'test_finalpass', 'test_cleanpass',
                              'test_allpass', 'test1of100_cleanpass', 'test1of100_finalpass']
        self.pairname_to_Limgname = lambda pairname: osp.join(self.root, pairname)
        self.pairname_to_Rimgname = lambda pairname: osp.join(self.root, pairname).replace('/left/', '/right/')
        self.pairname_to_Ldispname = lambda pairname: osp.join(self.root, pairname).replace('/frames_finalpass/',
                                                                                            '/disparity/').replace(
            '/frames_cleanpass/', '/disparity/')[:-4] + '.pfm'
        self.pairname_to_str = lambda pairname: pairname[:-4]
    ...
    def _build_cache(self):
        ...
        # assert len(trainpairs) == 22390, "incorrect parsing of pairs in SceneFlow"
        ...
        tosave = {'train_finalpass': trainpairs,
                  ...
                  'test1of100_cleanpass': list(
                      map(lambda x: x.replace('frames_finalpass', 'frames_cleanpass'), test1of100pairs)),
                  }
...
class Md21Dataset(StereoDataset):
    ...
    def _build_cache(self):
        ...
        for s in seqs:
            ...
            trainpairs += [s + '/ambient/' + b + '/' + a for b in sorted(os.listdir(osp.join(self.root, s, 'ambient')))
                           for a in sorted(os.listdir(osp.join(self.root, s, 'ambient', b))) if a.startswith('im0')]
...
class MdEval3Dataset(StereoDataset):
    ...
    def _prepare_data(self):
        ...
        self._set_root()
        assert self.split in [s + '_' + r for s in ['train', 'subtrain', 'subval', 'test', 'all'] for r in
                              ['full', 'half', 'quarter']]
        ...
        self.pairname_to_Ldispname = lambda pairname: None if pairname.startswith('test') else osp.join(self.root,
                                                                                                        pairname,
                                                                                                        'disp0GT.pfm')
        self.pairname_to_str = lambda pairname: pairname
        self.load_disparity = _read_middlebury_disp
        # for submission only
        self.submission_methodname = "UniTT-Stereo"
        self.submission_sresolution = 'F' if self.split.endswith('_full') else (
            'H' if self.split.endswith('_half') else 'Q')
    ...
    def _build_cache(self):
        ...
        assert len(trainpairs) == 15 and len(testpairs) == 15 and len(subvalpairs) == 1 and len(
            subtrainpairs) == 14 and len(allpairs) == 30, "incorrect parsing of pairs in Middlebury Eval v3"
        tosave = {}
        for r in ['full', 'half', 'quarter']:
            tosave.update(**{'train_' + r: trainpairs, 'subtrain_' + r: subtrainpairs, 'subval_' + r: subvalpairs,
                             'test_' + r: testpairs, 'all_' + r: allpairs})
        return tosave
    ...
    def submission_save_pairname(self, pairname, prediction, outdir, time):
        ...
        outfile = os.path.join(outdir,
                               pairname.split('/')[0].replace('train', 'training') + self.submission_sresolution,
                               pairname.split('/')[1], 'disp0' + self.submission_methodname + '.pfm')
...
class ETH3DLowResDataset(StereoDataset):
    ...
    def _prepare_data(self):
        ...
        self.pairname_to_Limgname = lambda pairname: osp.join(self.root, pairname, 'im0.png')
        self.pairname_to_Rimgname = lambda pairname: osp.join(self.root, pairname, 'im1.png')
        self.pairname_to_Ldispname = None if self.split == 'test' else lambda pairname: None if pairname.startswith(
            'test/') else osp.join(self.root, pairname.replace('train/', 'train_gt/'), 'disp0GT.pfm')
        self.pairname_to_str = lambda pairname: pairname
...
class BoosterDataset(StereoDataset):
    ...
    def _prepare_data(self):
        self.name = "Booster"
        self._set_root()
        assert self.split in ['train_balanced', 'test_balanced', 'subtrain_balanced',
                              'subval_balanced']  # we use only the balanced version
        self.pairname_to_Limgname = lambda pairname: osp.join(self.root, pairname)
        self.pairname_to_Rimgname = lambda pairname: osp.join(self.root, pairname).replace('/camera_00/', '/camera_02/')
        self.pairname_to_Ldispname = lambda pairname: osp.join(self.root, osp.dirname(pairname),
                                                               '../disp_00.npy')  # same images with different colors, same gt per sequence
    ...
    def _build_cache(self):
        ...
        trainpairs = ['train/balanced/' + s + '/camera_00/' + imname for s in trainseqs for imname in
                      sorted(os.listdir(self.root + 'train/balanced/' + s + '/camera_00/'))]
        testpairs = ['test/balanced/' + s + '/camera_00/' + imname for s in
                     sorted(os.listdir(self.root + 'test/balanced')) for imname in
                     sorted(os.listdir(self.root + 'test/balanced/' + s + '/camera_00/'))]
        ...
        tosave = {'train_balanced': trainpairs, 'test_balanced': testpairs, 'subtrain_balanced': subtrainpairs,
...
class SpringDataset(StereoDataset):
    ...
    def _prepare_data(self):
        ...
        self.pairname_to_Rimgname = lambda pairname: osp.join(self.root, pairname + '.png').replace('frame_right',
                                                                                                    '<frame_right>').replace(
            'frame_left', 'frame_right').replace('<frame_right>', 'frame_left')
        self.pairname_to_Ldispname = lambda pairname: None if pairname.startswith('test') else osp.join(self.root,
                                                                                                        pairname + '.dsp5').replace(
            'frame_left', 'disp1_left').replace('frame_right', 'disp1_right')
        self.pairname_to_str = lambda pairname: pairname
        self.load_disparity = _read_hdf5_disp

    def _build_cache(self):
        trainseqs = sorted(os.listdir(osp.join(self.root, 'train')))
        trainpairs = [osp.join('train', s, 'frame_left', f[:-4]) for s in trainseqs for f in
                      sorted(os.listdir(osp.join(self.root, 'train', s, 'frame_left')))]
        testseqs = sorted(os.listdir(osp.join(self.root, 'test')))
        testpairs = [osp.join('test', s, 'frame_left', f[:-4]) for s in testseqs for f in
                     sorted(os.listdir(osp.join(self.root, 'test', s, 'frame_left')))]
        ...
        assert len(trainpairs) == 5000 and len(testpairs) == 2000 and len(subtrainpairs) == 4904 and len(
            subvalpairs) == 96, "incorrect parsing of pairs in Spring"
        tosave = {'train': trainpairs, 'test': testpairs, 'subtrain': subtrainpairs, 'subval': subvalpairs}
        return tosave
    ...
    def submission_save_pairname(self, pairname, prediction, outdir, time):
        assert prediction.ndim == 2
        assert prediction.dtype == np.float32
        outfile = os.path.join(outdir, pairname + '.dsp5').replace('frame_left', 'disp1_left').replace('frame_right',
                                                                                                       'disp1_right')
        os.makedirs(os.path.dirname(outfile), exist_ok=True)
...
class Kitti12Dataset(StereoDataset):
    ...
    def _prepare_data(self):
        ...
        self.pairname_to_Rimgname = lambda pairname: osp.join(self.root, pairname.replace('/colored_0/',
                                                                                          '/colored_1/') + '_10.png')
        self.pairname_to_Ldispname = None if self.split == 'test' else lambda pairname: osp.join(self.root,
                                                                                                 pairname.replace(
                                                                                                     '/colored_0/',
                                                                                                     '/disp_occ/') + '_10.png')
...
class Kitti15Dataset(StereoDataset):
    ...
    def _prepare_data(self):
        ...
        self.pairname_to_Limgname = lambda pairname: osp.join(self.root, pairname + '_10.png')
        self.pairname_to_Rimgname = lambda pairname: osp.join(self.root,
                                                              pairname.replace('/image_2/', '/image_3/') + '_10.png')
        self.pairname_to_Ldispname = None if self.split == 'test' else lambda pairname: osp.join(self.root,
                                                                                                 pairname.replace(
                                                                                                     '/image_2/',
                                                                                                     '/disp_occ_0/') + '_10.png')
    ...
    def _build_cache(self):
        ...
        assert len(trainseqs) == 200 and len(subtrainseqs) == 195 and len(subvalseqs) == 5 and len(
            testseqs) == 200, "incorrect parsing of pairs in Kitti15"
        tosave = {'train': trainseqs, 'subtrain': subtrainseqs, 'subval': subvalseqs, 'test': testseqs}
        return tosave
...
def _read_drivingstereo_disp(filename, coef=256.0):
    disp = np.asarray(Image.open(filename))
    disp = disp.astype(np.float16) / coef
    disp[disp == 0.0] = np.inf
    return disp


def _read_tartanair_disp(filename): return 80 / np.load(filename)


def _read_sintel_disp(filename, coef=1.0):
    disp = np.asarray(Image.open(filename))
    disp = disp.astype(np.float32)[:, :, 0] / coef
    disp[disp == 0.0] = np.inf
    return disp
