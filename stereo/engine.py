...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module, criterion2: torch.nn.Module,
                    criterion3: torch.nn.Module, metrics: torch.nn.Module,
    ...
    for data_iter_step, (image1, image2, gt, pairname) in enumerate(
            metric_logger.log_every(data_loader, print_freq, header, max_iter=iter_per_epoch)):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            prediction, out, mask, target, feat1, feat2, ratio, decout = model(image1, image2, epoch, args, train=1)
            prediction, conf = split_prediction_conf(prediction, criterion.with_conf)
            batch_metrics = metrics(prediction.detach(), gt)
            depthLoss = criterion(prediction, gt) if conf is None else criterion(prediction, gt, conf)
            reconLoss = criterion2(out, mask, target)
            threshold = np.tanh(reconLoss.cpu().detach().numpy())
            consistLoss = criterion3(gt, (feat1, feat2), epoch, args, image1, model.patch_embed.patch_size[0], mask)
            ...
            if ratio == 0.0:
                loss = depthLoss 
                reconLoss = torch.zeros_like(reconLoss)
                consistLoss = torch.zeros_like(consistLoss)
            else:
                if threshold <= 0.4:
                    loss = loss_weight[0] * depthLoss + loss_weight[1] * reconLoss + loss_weight[2] * consistLoss
                else:
                    loss = loss_weight[1] * reconLoss + loss_weight[2] * consistLoss

        loss_value = loss.item()
        depthLoss_value = depthLoss.item()
        reconLoss_value = reconLoss.item()
        consistLoss_value = consistLoss.item()
        ...
        metric_logger.update(depthLoss=depthLoss_value)
        metric_logger.update(reconLoss=reconLoss_value)
        metric_logger.update(consistLoss=consistLoss_value)
        ...
        time_to_log = ((data_iter_step + 1) % (
                    args.tboard_log_step * accum_iter) == 0 or data_iter_step == len_data_loader - 1)
    ...
    for didx, data_loader in enumerate(data_loaders):
        ...
        for data_iter_step, (image1, image2, gt, pairname) in enumerate(
            ...
            if dname.startswith('Spring'):
                ...
                gt = (gt[:, :, 0::2, 0::2] + gt[:, :, 0::2, 1::2] + gt[:, :, 1::2, 0::2] + gt[:, :, 1::2,
                                                                                           1::2]) / 4.0  # we approximate the gt based on the 2x upsampled ones

            with torch.inference_mode():
                prediction, tiled_loss, c, decout, _, _, _ = tiled_pred(model, criterion, image1, image2, gt, conf_mode=conf_mode,
                                                       overlap=args.val_overlap, crop=crop,
                                                       with_conf=criterion.with_conf)
                batch_metrics = metrics(prediction.detach(), gt)
                loss = criterion(prediction.detach(), gt) if not criterion.with_conf else criterion(prediction.detach(),
                                                                                                    gt, c)
                loss_value = loss.item()
...
def _resize_stereo(data, new_size):
    assert data.ndim == 4
...
def tiled_pred(model, criterion, img1, img2, gt,
    ...
    if do_change_scale:
        ...
        if gt is not None: gt = _resize_stereo(gt, new_size)
        H, W = img1.shape[2:4]
    ...
    accu_c = img1.new_zeros(
        (B, H, W))  # accumulate the weighted sum of confidences ; not so useful except for computing some losses
    ...
    for sy1, sx1, sy2, sx2, aligned in crop_generator():
        ...
        pred, decout, enc_attn_scores, attn_scores, cross_attn_scores = model(_crop(img1, sy1, sx1), _crop(img2, sy2, sx2))
        ...
        if criterion is not None and gt is not None:
            tiled_losses.append(
                criterion(pred, gtcrop).item() if predconf is None else criterion(pred, gtcrop, predconf).item())
    ...
    if do_change_scale:
        pred = (_resize_stereo(pred, original_size))

    if return_time:
        return pred, torch.mean(torch.tensor(tiled_losses)), c, time, decout, enc_attn_scores, attn_scores, cross_attn_scores
    return pred, torch.mean(torch.tensor(tiled_losses)), c, decout, enc_attn_scores, attn_scores, cross_attn_scores
